package com.andres.curso.springboot.id.factura.springboot_di_factura;

import java.util.Arrays;
import java.util.List;

import org.springframework.context.annotation.Bean;
import org.springframework.context.annotation.Configuration;
import org.springframework.context.annotation.Primary;
import org.springframework.context.annotation.PropertySource;

import com.andres.curso.springboot.id.factura.springboot_di_factura.models.Item;
import com.andres.curso.springboot.id.factura.springboot_di_factura.models.Product;

@Configuration
@PropertySource("classpath:data.properties")
public class AppConfig {
    
    @Bean
    List<Item> itemInvoice(){
        Product p1=new Product("camara sony",800);
        Product p2=new Product("Video Camera",1200);
        return Arrays.asList(new Item(p1,2),new Item(p2,4));   
    }

     @Bean
     @Primary
    List<Item> itemInvoiceOffice(){
        Product p1=new Product("Monitor Sony 24",700);
        Product p2=new Product("Notebook Razer",2200);
        Product p3=new Product("Impresora HP  24",800);
        Product p4=new Product("Escritorio de Oficina",900);
        return Arrays.asList(new Item(p1,4),new Item(p2,6),new Item(p3,1),new Item(p4,4));   
    }
}
